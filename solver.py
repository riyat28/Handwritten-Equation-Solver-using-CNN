import re #Regular expression operations are used for string manipulation
import sympy as sym
from keras.models import model_from_json#Loading a trained model from JSON.
import numpy as np
import cv2
...
#This function loads a pre-trained neural network model from a JSON file and associated weights.
def load_model():
    with open('solver_model.json', 'r') as json_file:
        solver_model = model_from_json(json_file.read())
    solver_model.load_weights("solver_model_weights.h5")

    return solver_model


#This function uses the loaded neural network model to classify segmented symbols from the image.
def extract(img):
...
def solve_linear_equation(equation):
    # Replace 'dx' with 'd*x' to handle cases like '2x'
    equation = re.sub(r"(\d)(x)", r"\1*\2", equation)
     # Replace 'x3' with 'x**3' to handle cases like 'x2'
